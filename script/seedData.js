    ...
    try {
        ...
        for (const user of users) {
            await new Promise((resolve, reject) => {
                db.run(`INSERT INTO users (username, email, password, role, created_at, updated_at) 
        ...
        for (const tag of tags) {
            await new Promise((resolve, reject) => {
                db.run(`INSERT INTO tags (name, description, created_at) VALUES (?, ?, datetime('now'))`, [tag.name, tag.description], function (err) {
                    if (err)
                        reject(err);
                    else
                        resolve(this.lastID);
        ...
        for (const question of questions) {
            const questionId = await new Promise((resolve, reject) => {
                db.run(`INSERT INTO questions (title, description, user_id, votes, created_at, updated_at) 
           VALUES (?, ?, ?, ?, datetime('now'), datetime('now'))`, [question.title, question.content, question.user_id, question.votes], function (err) {
                    if (err)
                        reject(err);
                    else
                        resolve(this.lastID);
        ...
        for (const answer of answers) {
           ...
           VALUES (?, ?, ?, ?, ?, datetime('now'), datetime('now'))`, [answer.content, answer.question_id, answer.user_id, answer.vote_count, answer.is_accepted], function (err) {
                    if (err)
                        reject(err);
                    else
                        resolve(this.lastID);
