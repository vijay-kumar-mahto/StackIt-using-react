...
const seedData = async () => {
  ...
  try {
    ...
    for (const tag of tags) {
      await new Promise((resolve, reject) => {
        db.run(
          `INSERT OR IGNORE INTO tags (name, description, created_at) VALUES (?, ?, datetime('now'))`,
    ...
    // Large set of questions (120 total)
    const questionTemplates = [
      ...
      // Additional diverse questions for variety
    ...
    // Shuffle questions and take 120
    ...
    for (let i = 0; i < shuffledQuestions.length; i++) {
      ...
      // Create question with random data
      const questionId = await new Promise<number>((resolve, reject) => {
        db.run(
          `INSERT INTO questions (title, description, user_id, votes, views, created_at, updated_at) 
           VALUES (?, ?, ?, ?, ?, datetime('now', '-' || ? || ' days'), datetime('now', '-' || ? || ' days'))`,
          [question.title, question.content, randomUserId, randomVotes, randomViews, getRandomInt(0, 30), getRandomInt(0, 30)],
          function(err) {
            if (err) reject(err);
    ...
    // Generate answers for 80 questions (leaving 40 unanswered)
    ...
    for (const questionId of answeredQuestionIds) {
      // Generate 1-3 answers per question
        ...
        await new Promise((resolve, reject) => {
          db.run(
            `INSERT INTO answers (content, question_id, user_id, votes, is_accepted, created_at, updated_at) 
             VALUES (?, ?, ?, ?, ?, datetime('now', '-' || ? || ' days'), datetime('now', '-' || ? || ' days'))`,
            [answerContent, questionId, randomUserId, randomVotes, isAccepted, getRandomInt(0, 25), getRandomInt(0, 25)],
            function(err) {
              if (err) reject(err);
              else resolve(this.lastID);
            }
